...
input ushort           InpStopLoss        = 50;             // Stop Loss in pip (1.00045-1.00055 = 1 pip)
input ushort           InpTakeProfit      = 50;             // Take Profit in pip
input ushort           InpTrailingStop    = 5;              // Trailing Stop (min distance from price to Stop Loss in pip
input ushort           InpTrailingStep    = 5;              // Trailing Step in pip
...
input double           InpVolumeLotOrRisk = 1.0;            // value for "Money management"
...
input string           InpTradeComment    = "RSI EA v2";    //
...
int OnInit() {
   ...
   if (handle_iRSI == INVALID_HANDLE) {
      ...
      PrintFormat("Failed to create handle of the iRSI indicator for the symbol %s/%s, error code %d", m_symbol.Name(), EnumToString(Period()), GetLastError());
      return(INIT_FAILED);
   }
...
void OnTick() {
   ...
   */
   ...
   if (CloseBySignal) {
      if (count_buys  > 0 && rsi_0 < RsiSellLevel && rsi_1 > RsiSellLevel) ClosePositions(POSITION_TYPE_BUY);
      if (count_sells > 0 && rsi_0 > RsiBuyLevel  && rsi_1 < RsiBuyLevel)  ClosePositions(POSITION_TYPE_SELL);
   }

   // open position
   if (TimeControl()) {
      if (OpenBUY  && count_buys  < 1 && rsi_0 > RsiBuyLevel  && rsi_1 < RsiBuyLevel)  OpenPosition(POSITION_TYPE_BUY);
      if (OpenSELL && count_sells < 1 && rsi_0 < RsiSellLevel && rsi_1 > RsiSellLevel) OpenPosition(POSITION_TYPE_SELL);
   }
   return;
}
...
bool CheckVolumeValue(double volume, string &error_description) {
   ...
   int ratio = MathRound(volume/volume_step);
   if (MathAbs(ratio*volume_step-volume)>0.0000001) {
      error_description = StringFormat("Volume is not a multiple of the minimal step SYMBOL_VOLUME_STEP=%.2f, the closest correct volume is %.2f", volume_step, ratio*volume_step);
      return(false);
   }
...
void OpenBuy(double sl, double tp) {
   ...
   if (free_margin_check > margin_check) {
      if (m_trade.Buy(long_lot, m_symbol.Name(), m_symbol.Ask(), sl, tp, InpTradeComment)) {
         if (m_trade.ResultDeal() == 0) {
            ...
            //}
            Print("#1 Buy -> false. Result Retcode: ", m_trade.ResultRetcode(), ", description of result: ", m_trade.ResultRetcodeDescription());
            ...
            //   m_waiting_order_ticket = m_trade.ResultOrder();
            //}
            Print("#2 Buy -> true. Result Retcode: ", m_trade.ResultRetcode(), ", description of result: ", m_trade.ResultRetcodeDescription());
            PrintResultTrade(m_trade, m_symbol);
         }
      }
      else {
         Print("#3 Buy -> false. Result Retcode: ", m_trade.ResultRetcode(), ", description of result: ", m_trade.ResultRetcodeDescription());
         PrintResultTrade(m_trade, m_symbol);
      }
   }
   else {
...
void OpenSell(double sl, double tp) {
   ...
   if (free_margin_check > margin_check) {
      if (m_trade.Sell(short_lot, m_symbol.Name(), m_symbol.Bid(), sl, tp, InpTradeComment)) {
         if (m_trade.ResultDeal() == 0) {
            //if (m_trade.ResultRetcode() == 10009) {
            //   m_waiting_transaction = true;                    // "true" -> it's forbidden to trade, we expect a transaction
            //   m_waiting_order_ticket = m_trade.ResultOrder();
            //}
            Print("#1 Sell -> false. Result Retcode: ", m_trade.ResultRetcode(), ", description of result: ", m_trade.ResultRetcodeDescription());
            PrintResultTrade(m_trade, m_symbol);
         }
         else {
            //if (m_trade.ResultRetcode() == 10009) {
         ...
         Print("#3 Sell -> false. Result Retcode: ", m_trade.ResultRetcode(), ", description of result: ", m_trade.ResultRetcodeDescription());
         PrintResultTrade(m_trade,m_symbol);
      }
   }
   else {
                  ...
                  if (m_position.StopLoss() < m_position.PriceCurrent()-ExtTrailingStop-ExtTrailingStep) {
                     if (ExtTrailingStop >= stop_level) {
                        if (!m_trade.PositionModify(m_position.Ticket(), m_symbol.NormalizePrice(m_position.PriceCurrent()-ExtTrailingStop), m_position.TakeProfit())) {
                           Print("Modify ", m_position.Ticket(), " Position -> false. Result Retcode: ", m_trade.ResultRetcode(), ", description of result: ",m_trade.ResultRetcodeDescription());
                        }
                        RefreshRates();
